    ...
    try:
        ...
        query = """
            ...
            RETURNING 
                id;
        """
        revoked = await database.fetch_one(query, {"refresh_token": refresh_token})
        if revoked is None:
            raise TokenNotFound
...
async def refresh(
    database: Connection = None, refresh_token: str = None, new_access_token: str = None
) -> stokens.TokenGet:
    ...
    try:
        query = """
            ...
            AND
                is_revoked = FALSE;
        """
        refresh = await database.fetch_one(query, {"refresh_token": refresh_token})
        if refresh is None:
            raise TokenNotFound
